...
def getInfile():
    filename="/Users/alyshareinard/Dropbox/python/hangman/wordlistfrench.txt"
    filename2="~\Dropbox\python\hangman\wordlistfrench.txt"
    current_dir=os.getcwd()
    infile_name=current_dir+"\wordlistfrench.txt"
    ...
    try:
        ...
        with open(infile_name, 'r'): infile_name = 'wordlistfrench.txt'
    except IOError:
...
def chooseWord(infile_name):
    ...
    english_word, french_word=chosen_word.split(',')  
...
def main(win, infile_name):
    ...
    eng_word, word = chooseWord(infile_name)
    word_len=len(word)
